...
&  \text{s.t. } T_{\rm calc}^{\rm MPC}({\theta}) \leq \eta {\Ts} 
\end{align*}
...
$$\theta_{n+1} = \arg \min_{\theta \in \Theta} \hat J(\theta) - \alert{\delta} z(x)$$
\end{enumerate}
 \column{.45\textwidth}
 	\centering
 \begin{figure}
...
\begin{itemize}
\item Higher- and lower-level settings tuned altogether to maximize performance, while keeping the design feasible
